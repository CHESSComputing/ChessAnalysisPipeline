...
(subclassed into `Reader`\\s, `Processor`\\s, and `Writer`\\s).
A pipeline uses a sequence of `PipelineItem`\\s to execute a data
processing workflow where the data returned by one `PipelineItem`
becomes input for the next one.
