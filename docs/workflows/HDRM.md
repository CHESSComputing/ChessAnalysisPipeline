...
1. Install a local version of the CHAP package according to the [installation instructions](installation_ref).
...
Create a workflow `pipeline.yaml` file according to the [CHAP pipeline instructions](chap_pipeline_ref). A generic pipeline input file is as follows (note that spaces and indentation are important in `.yaml` files):
```
config:
